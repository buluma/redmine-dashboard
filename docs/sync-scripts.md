...
This document describes the operational scripts in `scripts/` for syncing and
maintaining data between Redmine and the local database.
...
Performs a full paginated sync of **all** Redmine issues (`status_id=*`)
directly to the database, outside the web app sync-job queue. Use this for large
datasets (e.g. ~109K tickets) when `sync/manual-pull` is too heavy for
in-process jobs.
...
# Limit to a subset (10 pages x 100 issues/page by default) — this still
# writes to the database, it's not a dry-run; there is no dry-run flag
...
- Ignores `REDMINE_SYNC_ISSUE_SCOPE` runtime behavior; always syncs
  `status_id=*`.
- Uses resumable checkpoints in `/tmp/redmine-sync-all-progress.json` by
  default.
...
Syncs only issues **assigned to** or **authored by** the authenticated user.
...
**Note:** Some queries may not be accessible via REST API (private queries or
those requiring special permissions).

### `scripts/sync-query-details.js`

Syncs children and time entries for all issues returned by one or more saved
queries.
...
Extracts unique users from Redmine issues (author, assigned_to) and stores them
in the `RedmineUser` table.

```bash
node scripts/sync-users.js
```

**Usage:** The synced users are used for issue assignment in the Quick Actions
panel.

### `scripts/sync-enums.js`

Fetches Redmine enumerations (issue priorities, time entry activities, document
categories) and stores them in `RedmineEnumeration`.
...
Quick check of database state — total issues, parent-child breakdown, and issues
per user.
...
**All syncs are strictly additive (upsert only).** The destructive `deleteMany`
operation that previously ran during system sync jobs has been disabled to
prevent accidental data loss.

**Never run `DELETE FROM "Issue"` manually** unless you're intentionally
