#!/usr/bin/env node
/**
 * Sync all child issues for a specific parent issue.
 * Uses Redmine API: /issues.json?parent_id=<parentId>
 * 
 * Usage: node scripts/sync-parent-children.js <parent_issue_id>
 * Example: node scripts/sync-parent-children.js 97459
 */
...
const parentIssueId = parseInt(process.argv[2], 10);
if (isNaN(parentIssueId)) {
  console.error("Usage: node scripts/sync-parent-children.js <parent_issue_id>");
  console.error("Example: node scripts/sync-parent-children.js 97459");
  process.exit(1);
}
...
async function fetchPage(offset, retries = 3) {
  const url = `${REDMINE_BASE_URL}/issues.json?key=${REDMINE_API_KEY}&limit=${PAGE_SIZE}&offset=${offset}&parent_id=${parentIssueId}&status_id=*`;
...
function buildPayload(userId, issueRaw, baseUrl) {
  ...
  return {
    ...
    parentIssueId: nestedId(issueRaw.parent) ?? parentIssueId,
    parentIssueLabel: nestedId(issueRaw.parent) ? `#${nestedId(issueRaw.parent)}` : `#${parentIssueId}`,
...
async function main() {
  ...
  console.log(`📂 Syncing children of parent issue #${parentIssueId}`);
  ...
  console.log(`📋 Total child issues: ${totalCount.toLocaleString()} (${totalPages} pages)\n`);

  if (totalCount === 0) {
    console.log("✅ No child issues found for this parent.");
  ...
  const withParent = await prisma.issue.count({ where: { userId, parentIssueId } });

  console.log(`\n\n✅ Done in ${elapsed}s`);
  console.log(`📈 Total issues in DB for user: ${totalInDb.toLocaleString()}`);
  console.log(`📎 Issues under #${parentIssueId}: ${withParent.toLocaleString()}`);
  console.log(`📥 Child issues synced: ${totalUpserted.toLocaleString()}`);
